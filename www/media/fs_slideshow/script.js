jQuery(document).ready(function() {
    ...
    if (jQuery("#images .other-images").length > 0) {
        ...
        var initZ = jQuery(".inline-species-image").first();
        if (!initZ.hasClass("missing_zoomify")) {
            jQuery("#zoomify-factsheet").removeClass("hidden");
            ...
            jQuery("#zoomify-factsheet").height(h+21);

        } else {
            initZ = jQuery(".inline-species-image:not(.missing_zoomify)").first();
            jQuery("#zoomify-factsheet").removeClass("hidden");
            Z.showImage("zoomify-factsheet", initZ.text(), "zToolbarVisible=1&zNavigatorVisible=2&zSkinPath=/media/zoomify/Skins/Default");
